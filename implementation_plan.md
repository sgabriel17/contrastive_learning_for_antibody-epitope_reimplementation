...
- Frozen baseline notebook (`notebooks/01_frozen_baseline.ipynb`): loads pre-computed AbLangRBD1 embeddings + frozen AntiBERTy/ESM-2, computes binary AUROC and t-SNE
- Baseline results: AbLangRBD1=0.812 (binary AUROC, test-vs-test), AntiBERTy(frozen)=0.546, ESM-2(frozen)=0.547
...
# All checkpoints will be saved here, not to ephemeral /content scratch.
DRIVE_OUTPUT = "/content/drive/MyDrive/6.8711/checkpoints"  # adjust path as needed
```
...
BRANCH = "test1"  # or whichever branch you are working on

if not os.path.exists(REPO_DIR):
    !git clone https://github.com/sgabriel17/contrastive_learning_for_antibody-epitope_reimplementation.git {REPO_DIR}
%cd {REPO_DIR}
...
EPOCHS     = 400
BATCH_SIZE = 256
GRAD_ACCUM = 1               # increase to 2 or 4 if OOM on T4
RUN_NAME   = f"{ENCODER}_run1"
OUTPUT_DIR = f"{DRIVE_OUTPUT}/{RUN_NAME}"
RESUME     = ""              # set to f"{OUTPUT_DIR}/checkpoint_latest.pt" to resume
```
...
epochs      = [m["epoch"] for m in metrics if "train_vs_val" in m]
auroc_vals  = [m["train_vs_val"]["weighted_auroc"] for m in metrics if "train_vs_val" in m]
train_loss  = [m["train_loss"] for m in metrics]
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
ax1.plot(train_loss); ax1.set(title="Train loss", xlabel="Epoch")
ax2.plot(epochs, auroc_vals); ax2.set(title="Val weighted AUROC (train-vs-val)", xlabel="Epoch")
plt.tight_layout(); plt.savefig(f"{OUTPUT_DIR}/training_curves.png", dpi=150)
plt.show()
```
...
- `metrics.json` — written every epoch; readable without loading weights.
- `run_config.json` — CLI args saved at start of run.

To resume after disconnect: set `RESUME` in Cell 5 and re-run Cells 5–6.
...
**ICML template:** [https://github.com/icml-compbio/icml-compbio.github.io/raw/master/2022/icml_wcb_2022.zip](https://github.com/icml-compbio/icml-compbio.github.io/raw/master/2022/icml_wcb_2022.zip)
